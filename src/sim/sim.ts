...
// A star is an INFINITE SLICE: apex at the arc point, mouth opening forever
// upward. The catch fans WIDER the higher the chest clears the arc with NO
// ceiling, so crossing the slice anywhere above the arc collects and no pop
// can overfly it. At or below the arc the catch is the usual generous disc;
// a low, unpopped flight passing beneath still misses, so reaching the arc
// is still the feat. Exported because the render layer draws the funnel
// with the SAME numbers — the shape you see is the shape that collects.
export const BONUS_RADIUS = 1.7; // stars are generous — reaching them was the feat
export const BONUS_CONE_FLARE = 0.5; // extra catch radius per meter cleared above the arc
...
export function stepSim(sim: Sim, input: SkierInput): SimEvent[] {
  ...
  if (s.tumbling === 0) {
    ...
    for (const star of sim.terrain.bonusesNear(s.z)) {
      ...
      const reach = BONUS_RADIUS + cleared * BONUS_CONE_FLARE; // the slice widening upward, forever
      if (dx * dx + dz * dz < reach * reach && dy < BONUS_RADIUS) {
