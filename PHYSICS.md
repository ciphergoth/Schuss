...
| Reference drift         | Measured 2026-07-06 (archetype work): a hands-off rider at exact reference conditions threads only ~15% of ×5 stars — real flights run ~2–4m below/short of the hand-integrated arc on venues with crud, bends, grade swings, or scooped landings (clean-cruise venues thread fine). Players compensate with pace (a faster flight flattens onto a low star) and steering, so stars stay collectable in play — but the instrument is optimistic. Honest fix: integrate the reference flight with the REAL `stepSkier` band dynamics instead of the hand-rolled march. Gate tests pin the laws at venues where a reference ride verifiably threads (`findThreadableVenue`). | ⚠️ calibration item            |
